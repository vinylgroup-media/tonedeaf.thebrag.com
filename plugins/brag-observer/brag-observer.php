...
class BragObserver
{
    ...
    protected array $artist_keywords;
    ...
    public function __construct()
    {
        ...
        // AJAX
        add_action('wp_ajax_subscribe_observer', [$this, 'ajax_subscribe_observer']);
        add_action('wp_ajax_nopriv_subscribe_observer', [$this, 'ajax_subscribe_observer']);
    ...
    */
    ...
    private static function callAPI($method, $url)
    {
        $curl = curl_init();
        switch ($method) {
            case "POST":
...
*/
    ...
    public function add_observer_topic_field_post($post): void
    {
...
*/
    ...
    public function get_observer_topics($topic = NULL)
    {
        ...
        if ($responseBody) {
          ...
          $topics = isset($responseJson->success) && $responseJson->success ? $responseJson->data : [];
        } else {
          $topics = '';
        }
        */
    ...
    */
    ...
    public function add_observer_topic_field($post): void
    {
        $topics = wp_list_pluck($this->get_observer_topics(), 'title', 'id');
        asort($topics);
    ...
    */
    ...
    public function save_genre_meta($term_id, $tt_id): void
    {
    ...
    */
    ...
    public function edit_genre_field($term): void
    {
        $topics = wp_list_pluck($this->get_observer_topics(), 'title', 'id');
        asort($topics);
        ...
        <td>
            <div>
                Primary topic
                <label for="primary-observer-topic"></label>
                    ...
                    <tr>
                        ...
                        foreach ($topics as $id => $title) : ?>
                            <td style="padding: 5px;">
                                <label>
                                    <input type="checkbox" name="observer-topic[]"
    ...
    public function update_genre_meta($term_id): void
    {
    ...
    */
    ...
    public function shortcode_observer_tastemaker_form($atts)
    {
        ...
        $tastemaker_atts = shortcode_atts([
            'id' => NULL,
            'background' => '#e9ecef',
            'border' => '#fff',
            'width' => NULL
        ...
        wp_enqueue_script($this->plugin_name, plugin_dir_url(__FILE__) . '/js/scripts.min.js', ['jquery'], '20211014', true);
        $args = [
        ...
        if ($responseBody) {
            $responseJson = json_decode($responseBody);
            $form_html = isset($responseJson->success) && $responseJson->success ? $responseJson->data : '';
        } else {
            $form_html = '';
        }
    ...
    public function shortcode_observer_lead_generator_form($atts)
    {
        ...
        $lead_generator_atts = shortcode_atts([
            'id' => NULL,
            'background' => '#e9ecef',
            'border' => '#fff',
            'width' => NULL,
            ...
            if ($topics) {
                foreach ($topics as $topic) {
                    ...
                    foreach ($keywords as $keyword) {
                        ...
                        if (str_contains(strtolower(get_the_content()), $keyword)) {
                            $topic_id = $topic->id;
                            break;
                        }
                    }
                ...
                <div class="p-3 d-flex justify-content-center align-items-center">
                    <div>
                        ...
                        <form action="#" method="post" id="observer-subscribe-form<?php echo $post_id; ?>"
                              ...
                              class="observer-subscribe-form <?php echo !is_user_logged_in() ? 'd-none bg-white' : ''; ?>">
                            <div class="d-flex justify-content-start">
                                <input type="hidden" name="list" value="<?php echo $topic_id; ?>">
                                <?php if (!is_user_logged_in()) : ?>
                                    <label>
                                        <input type="email" name="email" class="form-control observer-sub-email"
                                               placeholder="Your email" value="">
                                    </label>
                                <?php endif; ?>
                                <div class="d-flex submit-wrap rounded">
                                    <input type="submit" value="Join" name="subscribe"
                                           class="button btn btn-primary rounded" style="color: #fff !important">
                                </div>
                            </div>
                        </form>
...
public function ajax_subscribe_observer(): void
{
...
public function save_tastemaker_review(): void
{
        ...
        if (defined('DOING_AJAX') && DOING_AJAX) :
            ...
            $tastemaker_id = $formData['id'] ?? null;
            ...
            if ($responseJson->success) {
                ...
                wp_send_json_error($responseJson->data);
            }
        endif;
    }
    ...
    */
    ...
    public function save_lead_generator_response(): void
    {
    ...
    */
    ...
    {
        register_rest_route('api/v1', '/observer/articles', [
            'methods' => 'GET',
            'callback' => [$this, 'get_articles_for_topic'],
        ]);

        register_rest_route('api/v1', '/observer/competitions', [
            'methods' => 'GET',
    ...
    */
     ...
     * @throws Exception
     */
    function get_articles_for_topic(): array
    {
    ...
    */
    ...
    public function _save_post($post_id, $post): void
    {

        if (str_contains($post->post_content, '[observer_lead_generator_form')) {
    ...
    */
    function get_competition_articles(): array
    {
